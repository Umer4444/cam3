...
var game = function(req, res) {
    ...
    res.render('tic-tac-toe.html', view);
};
...
module.exports = function(app, server) {
	...
	io.sockets.on("connection", function(socket) {
		...
		if (!game.player1.isInUse()) {
			...
			// inform the client
			io.sockets.to(socket.id).emit("player_id", {
				playerNumber: game.player1.getNumber(),
				playerUsername: game.player1.getUsername(),
			...
			io.sockets.to(socket.id).emit("player_id", {
				...
				playerUsername: game.player2.getUsername(),
		...
		if (game.player1.isInUse() && game.player2.isInUse()) {
			io.sockets.to(game.player1.getID()).emit("ready_to_play", {
				currentPlayer: game.currentPlayer.getNumber()
			});
			io.sockets.to(game.player2.getID()).emit("ready_to_play", {
				currentPlayer: game.currentPlayer.getNumber()
			});
		}
		...
		socket.on("clicked", function(data) {
			...
			io.sockets.to(game.player1.getID()).emit("space_claimed", {
				spaceID: data.spaceID,
				xo: game.currentPlayer.getXO()
			});
			io.sockets.to(game.player2.getID()).emit("space_claimed", {
				spaceID: data.spaceID,
				xo: game.currentPlayer.getXO()
			});
			...
			if (endTurnResult.winner) {
				...
				io.sockets.to(game.player1.getID()).emit("end_game", {
					winner: endTurnResult.winner
				});
				io.sockets.to(game.player2.getID()).emit("end_game", {
					winner: endTurnResult.winner
				});
		...
		socket.on("play_again", function() {
			...
			io.sockets.to(player.getID()).emit("waiting_for_player", {
		...
		socket.on("disconnect", function() {
			...
			if (game.player1.isInUse()) {
				io.sockets.to(game.player1.getID()).emit("waiting_for_player", {
					// we're hard coding player1 === X... is that okay?
					xScore: game.player1Score,
					oScore: game.player2Score
				});
			...
			if (game.player2.isInUse()) {
				io.sockets.to(game.player2.getID()).emit("waiting_for_player", {
					// we're hard coding player1 === X... is that okay?
					xScore: game.player1Score,
					oScore: game.player2Score
				});
    ...
    app.get('/initiator/:initiatorId/:initiatorUsername/opponent/:opponentId/:opponentUsername/:game', game);
    app.get('/opponent/:opponentId/:opponentUsername/initiator/:initiatorId/:initiatorUsername', game);
