 ...
 * @author l0co@wp.pl
 */
abstract class App_Payment_Gateway_Base {
	 ...
	 */
   // public function beginSession($amount, $description, $success_url, $error_url,$id_user,$user_type) {
	public function beginSession($amount, $description, $id_user, $user_type, $encrypt, $member_id, $chips, $rate, $plan) {       
		$loader = App_Payment_Registry::getInstance()->getLoader();
        
        //$payment = $loader->build($this->type, $amount, $description, $success_url, $error_url,$id_user,$user_type);
		$payment = $loader->build($this->type, $amount, $description, $id_user, $user_type, $encrypt, $member_id, $chips, $rate, $plan);
		$payment->save();
	...
	public function restoreSession($p) {
		if ($p instanceof App_Payment_Session_Interface) {
			$this->session = $p;
		} else {
	...
	public function doValidation(Zend_Controller_Request_Http $request) {
		if (!$this->session)
			throw new App_Payment_Exception("Payment session must be loaded to perform verification.");
		if (!$this->session->getStatus() == App_Payment_Session_Interface::STATUS_NEW)
			throw new App_Payment_Exception("This payment is not appropriated for finalizing (status=" .
				$this->session->getStatus() . ").");

		try {
			$result = $this->validate($request);
			$this->session->setStatus(App_Payment_Session_Interface::STATUS_DONE);
			App_Payment_Registry::getInstance()->getLoader()->save($this->session);
		} catch (App_Payment_Exception $e) {
			$this->session->status = App_Payment_Session_Interface::STATUS_CANCELLED;
			App_Payment_Registry::getInstance()->getLoader()->save($this->session);
			throw $e;
		}
	}
	...
	protected function buildUrl($action) {
		return "http://". App_Payment_Registry::getInstance()->serverpath . "/" .
    ...
    public function load($model){
        ...
        if(!isset($this->{$model})){
            ...
            if(!file_exists($modelUrl) && !file_exists($defaultModelUrl)) throw new App_Payment_Exception("The requested model does not exist in <u>".$modelUrl."</u>");
