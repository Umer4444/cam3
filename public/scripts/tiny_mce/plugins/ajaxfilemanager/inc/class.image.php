...
?><?php
     ...
     * @copyright this function originally come from Andy's php 
     */
    function showImage($type = '', $quality = '40')
    {   
        ...
        switch($type) 
        {
            ...
            case 'png':
                ...
                $func($this->_imgFinal, NULL, ($quality > 99 )? (int)($quality-1)/10 : (int)$quality/10);
                break;
            case 'jpg':
                $func($this->_imgFinal, NULL, $quality);
                break;
            case 'jpeg':
                $func($this->_imgFinal, NULL, $quality);
                break;
	...
	function resize( $new_x, $new_y, $constraint= false, $unchangeIfsmaller=false)
	{
		...
		if($new_x <=0 || $new_y <= 0)
		{
			$this->_debug('either of new width or height can be zero or less.');
		}else 
		{
    ...
    // hacks for transparency of png24 files
