<?php
/**
 * This file is generated automatically for table "order_address". Do not change
...
class BaseOrderAddressGrid extends \ZfTable\AbstractTable
{
    ...
    public function init()
    {
        ...
        $this->getHeader("edit")->getCell()->addDecorator("callable", array(
            "callable" => function($context, $record){
                return sprintf("<a href=\"/admin/crud/order-address/update/%s\">Edit</a>", $record->getId());//address_id
            }
        ));

        $this->getHeader("delete")->getCell()->addDecorator("callable", array(
            "callable" => function($context, $record){
                return sprintf("<a href=\"/admin/crud/order-address/delete/%s\">Delete</a>", $record->getId());
            }
        ));
    }
    ...
    protected function initFilters($query)
    {
        ...
        $value = $this->getParamAdapter()->getValueOfFilter('last_name');
        if ($value != null) {
            $query->where("last_name like '%".$value."%' ");
        }

        $value = $this->getParamAdapter()->getValueOfFilter('middle_name');
        if ($value != null) {
            $query->where("middle_name like '%".$value."%' ");
        }

        $value = $this->getParamAdapter()->getValueOfFilter('company_name');
        if ($value != null) {
